...
#include "tuto/robot_strat_tuto1.hpp"
...
int RobotStrat::read_yaml_conf(char *fname)
{
  ...
  /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
  /*++ TUTO1 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
  /*++ Lecture de la configuration de strat +++++++++++++++++++++++++++++++++*/
  /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
  ...
  section_node = yconf["section_tuto"];
  if (!section_node) 
  {
    printf ("ERROR: TUTO1: section 'section_tuto' does not exist!\n");
    return -1;
  }

  YAML::Node sub_section_node;
  sub_section_node = yconf["section_tuto"]["sub_section_tuto"];
  /*
    NOTE : equivalent code is valid :
    sub_section_node = section_node["sub_section_tuto"];
   */
  if (!sub_section_node) 
  {
    printf ("ERROR: TUTO1: sub_section 'sub_section_tuto' does not exist!\n");
    return -1;
  }

  YAML::Node str1_node;
  str1_node = sub_section_node["str1_field"];
  if (!str1_node) 
  {
    printf ("ERROR: TUTO1: 'str1_field' does not exist!\n");
    return -1;
  }
  else
  {
    my_str = (const char *) str1_node.as<std::string>().c_str();
    printf ("TUTO1: str1 = '%s'\n", my_str);
  }

  YAML::Node num1_node;
  num1_node = sub_section_node["num1_field"];
  if (!num1_node) 
  {
    printf ("ERROR: TUTO1: 'num1_field' does not exist!\n");
    return -1;
  }
  else
  {
    my_str = (const char *) num1_node.as<std::string>().c_str();
    int num1_val = atoi(my_str);
    printf ("TUTO1: num1 = %d\n", num1_val);
  }

  YAML::Node vector_list1_node;
  vector_list1_node = sub_section_node["vector_list1"];
  if (!vector_list1_node) 
  {
    printf ("ERROR: TUTO1: 'vector_list1' does not exist!\n");
    return -1;
  }
  else
  {
    printf ("TUTO1: vector_list1:\n");
    int n_items = vector_list1_node.size();
    for (int i=0; i<n_items; i++)
    {
      YAML::Node item_node = vector_list1_node[i];
      int x,y;
      my_str = item_node[0].as<std::string>().c_str();
      x = strtof(my_str, NULL);
      my_str = item_node[1].as<std::string>().c_str();
      y = strtof(my_str, NULL);
      printf ("  <%d,%d>\n", x, y);
    }
...
void RobotStrat::taskFunction()
{
  ...
  while(!m_stop_task)
  {
    ...
    /* FIXME : TODO : main strat code will go here.. */
