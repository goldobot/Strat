...
void MainWindow::setupGoldoDemo(QCustomPlot *customPlot)
{
  ...
  customPlot->addGraph(); // red line
  customPlot->graph(1)->setPen(QPen(Qt::red));
  QVector<double> box_x(4), box_y(4);
  box_x[0] =  50; box_y[0] =-140;
  box_x[1] =  50; box_y[1] = 140;
  box_x[2] = 300; box_y[2] = 140;
  box_x[3] = 300; box_y[3] =-140;
  customPlot->graph(1)->setData(box_x, box_y);

  customPlot->addGraph(); // red line
  customPlot->graph(2)->setPen(QPen(Qt::red));
  QVector<double> bbox_x(2), bbox_y(2);
  bbox_x[0] =  50; bbox_y[0] =-140;
  bbox_x[1] = 300; bbox_y[1] =-140;
  customPlot->graph(2)->setData(bbox_x, bbox_y);
